...
def main():
    class Window(Frame):
        ...
        def init_window(self):
            ...
            global i2
            i2 = IntVar()
            c2 = Checkbutton(self, text='Concatenar?',variable=i2)
            c2.pack()
            c2.select()
            ...
            c2.place(relx=0.5, rely=0.75, anchor=CENTER)
            
            
            # Botão de conversão
            convert.place(relx=0.50, rely=0.88, anchor=CENTER)
        ...
        def convertFiles(self):
            ...
            if not os.path.exists(path):
                ...
                if not ts_file:
                    ...
                    convertEngs = scriptgui.preplistas(engine_file,event_file,ts_file,path,deb,concatenardb)
        ...
        def ts(self):
            ts_filename = askopenfilename(parent=root,
                                   title='Selecione o arquivo de Troubleshoot', filetypes = [("Excel files","*.xlsx")])
            if ts_filename:
